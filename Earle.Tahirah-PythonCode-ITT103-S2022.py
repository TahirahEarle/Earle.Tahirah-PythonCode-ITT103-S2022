...
count=0
"""The condtion used to test the loop is the while true do, because there was no input thus far to test the loop againts since
it is set to true the loop that will continue to run until you explicitly breakout of it. Because True will always evaluate to True,
you have to force the loop to end when you want it to."""
while True:
  count+=1
  print('─' * 42,"Loop",count,'─' * 41) 
  ...
  # Since there can be only two errors that can be derived from if it's not the sale class it has to be the sale amount. 
  if sale_amount < 0: 
      error=("Sale Amount Error")
  else:
      error=("Sale Class Error")
      
  # Raise a value error if a negative sale amount or less than 0 is entered. So that the program doesn't calculate a negative commission. 
  try:
    if sale_amount <0 :
      ...
      print(ve)
      sale_amount=float(input("To try again please lease Re-enter sales amount :$"))
  ...
  # If none of the conditions was true. 
    ...
    print("There was an error -".upper(), error )
    print ("The program only assign a commission rate if you are apart of class 1 -3")
    print('─' * 91) 
